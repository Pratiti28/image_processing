...
def upload_image():
    ...
    elif operation_selection == 'watercolour':
...
def watercolour(decode_array_to_img):
    water_effect = cv2.stylization(decode_array_to_img, sigma_s=10, sigma_r=0.16)
...
def image_HDR(decode_array_to_img):
    HDR_effect = cv2.detailEnhance(decode_array_to_img, sigma_s=2000, sigma_r=0.3)
